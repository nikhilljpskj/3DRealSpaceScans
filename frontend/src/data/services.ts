import { TaxonomyType } from "data/types";


export const SERVICES: TaxonomyType[] = [
    {
      id: "1",
      href: "/3d-virtual-tours",
      name: "3D Virtual Tours for Real Estate",
      desc: "Immerse buyers in lifelike property walkthroughs with interactive 3D virtual tours. Elevate engagement and accelerate sales by showcasing every detail remotely.",
      taxonomy: "category",
      thumbnail: "/assets/services/3d-tour.jpg",
    },
    {
      id: "2",
      href: "/digital-twin",
      name: "Digital Twins for Construction & Architecture",
      desc: "Generate precise, scalable 3D models for construction projects, enabling real-time progress tracking, troubleshooting, and stakeholder collaboration.",
      taxonomy: "category",
      thumbnail: "/assets/services/digital-twin.jpg",
    },
    {
      id: "3",
      href: "/virtual-staging",
      name: "Virtual Staging",
      desc: "Transform empty spaces into beautifully furnished rooms, helping prospective buyers envision their future homes.",
      taxonomy: "category",
      thumbnail: "/assets/services/virtual-staging.jpg",
    },
    {
      id: "4",
      href: "/virtual-staging",
      name: "Scan to BIM",
      desc: "Transform empty spaces into beautifully furnished rooms, helping prospective buyers envision their future homes.",
      taxonomy: "category",
      thumbnail: "/assets/services/virtual-staging.jpg",
    },
    {
      id: "5",
      href: "/virtual-staging",
      name: "Scan to Plan",
      desc: "Transform empty spaces into beautifully furnished rooms, helping prospective buyers envision their future homes.",
      taxonomy: "category",
      thumbnail: "/assets/services/virtual-staging.jpg",
    },
    {
      id: "6",
      href: "/virtual-staging",
      name: "Facility Management Scanning",
      desc: "Transform empty spaces into beautifully furnished rooms, helping prospective buyers envision their future homes.",
      taxonomy: "category",
      thumbnail: "/assets/services/virtual-staging.jpg",
    },
    {
      id: "7",
      href: "/virtual-staging",
      name: "Large Object 3D Laser Scanning",
      desc: "Transform empty spaces into beautifully furnished rooms, helping prospective buyers envision their future homes.",
      taxonomy: "category",
      thumbnail: "/assets/services/virtual-staging.jpg",
    },
    {
      id: "8",
      href: "/virtual-staging",
      name: "Industrial 3D Laser Scanning",
      desc: "Transform empty spaces into beautifully furnished rooms, helping prospective buyers envision their future homes.",
      taxonomy: "category",
      thumbnail: "/assets/services/virtual-staging.jpg",
    },
    {
      id: "9",
      href: "/virtual-staging",
      name: "3D Laser Building Scanning Services",
      desc: "Transform empty spaces into beautifully furnished rooms, helping prospective buyers envision their future homes.",
      taxonomy: "category",
      thumbnail: "/assets/services/virtual-staging.jpg",
    },
  ];
  